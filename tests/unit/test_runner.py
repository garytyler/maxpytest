...
def test_runtests_default_calls_UserMax(tmpdir, mocker, mock_UserMax):
    ...
    mocker.patch.object(
        runner.maxcom.MXSPyCOM, "_get_default_exepaths", raise_runtime_error
    )
    runner.runtests(cwd=str(tmpdir), runnerarg=None, pytestargs=[], restart=None)
...
def test_runtests_with_restart_creates_restarter(mocker, tmpdir, tmpfile):
    ...
    create_restarter = mocker.patch.object(runner.callscripts, "create_restarter")

    runner.runtests(cwd=cwdpath, runnerarg=tmpexefile, pytestargs=[], restart=True)

    create_restarter.assert_called_once_with(launchscript=testcaller)


def test_runtests_default_no_max_installed_raises(
    mocker, tmpdir, patch_max_root_env_vars
):
...
@pytest.mark.parametrize(
    "called, not_called", [("MXSPyCOM", "UserMax"), ("UserMax", "MXSPyCOM")]
)
def test_runtests_with_exepath(mocker, tmpdir, called, not_called):

    called_filename = getattr(runner.maxcom, called).filename
    mock_called = mocker.patch.object(runner.maxcom, called, filename=called_filename)
    not_called_filename = getattr(runner.maxcom, not_called).filename
    mock_not_called = mocker.patch.object(
        runner.maxcom, not_called, filename=not_called_filename
    )
    runner.runtests(
        cwd=str(tmpdir),
